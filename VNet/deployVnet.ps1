...
if ($subscriptions.count -gt 1){
    Write-Host "Account has access to more than one subscription, please input subscription name or ID"
...
$ResourceGroupName = "RG-"+$appName+"-"+$ResourceGroupLocation
...
$depout = New-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName -TemplateFile $TemplateFilePath -TemplateParameterObject $VNETParams -DeploymentDebugLogLevel None;

Write-Host "ObectID: [$($depout.Outputs.vnetObjectID.Value)]"
Write-Host "VNETName: [$($depout.Outputs.vnetName.Value)]"
Write-Host "SubnetName: [$($depout.Outputs.subnetName.Value)]"
